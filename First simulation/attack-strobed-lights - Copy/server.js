...
//#endregion
    ...
    .updated(async (ctx) =>
     {
        ...
        return Promise.all([
            ...
            ctx.api.subscriptions.subscribeToDevices(ctx.config.lights, 'switch', 'switch.on', 'turnOnOffMotionHandler')
        ])
    })
    //#endregion
    ...
    .subscribedEventHandler('turnOnOffMotionHandler', (ctx, event) => 
    {
        ctx.api.devices.sendCommands(ctx.config.motionSensors, "motionSensor", "active")
        ctx.api.devices.sendCommands(ctx.config.motionSensors, "motionSensor", "inactive")
    })
