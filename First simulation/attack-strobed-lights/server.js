...
const client = new SmartThingsClient(new BearerTokenAuthenticator('4fdf3a12-e265-4704-9bb2-40d06a02b45f')); // PAT token
const add = false; 
...
//#endregion
    ...
    .scheduledEventHandler('checkMotion', async (ctx, event) => 
    {
        ...
        let quiet = othersQuiet (ctx, ctx.config.motionSensors, event.deviceId)
            .then ( thereIsMovement => { quiet = thereIsMovement; console.log (`There is movement?: ${thereIsMovement}`) });
        //console.log (`There is movement?: ${quiet}`);

        if (quiet)
        {
            const elapsed = client.devices.getCapabilityStatus(event.deviceId, 'main', 'motionSensor')
                .then (status => { elapsed = Date.now() - status.motion.timestamp.valueOf() });

            // ********** why with ctx doesn't work? ****************
            //const elapsed = ctx.api.devices.getCapabilityStatus(event.deviceId, 'main', 'motionSensor')
            //    .then (status => { elapsed = status })
...
client.devices.getCapabilityStatus('8710aeab-58a0-44bd-adad-74c83d9404b4', 'main', 'motionSensor')
    .then (status => {console.log (`status: ${status.motion.timestamp}`)})

//console.log (client.devices.getCapabilityStatus('8710aeab-58a0-44bd-adad-74c83d9404b4', 'main', 'motionSensor'))
